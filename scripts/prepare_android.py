    ...
    if elem.tag == 'activity':
        ...
        for elem2 in elem:
            if elem2.tag == 'intent-filter':
                ...
                for elem3 in elem2:
                    if elem3.tag == 'action':
                        if elem3.get('{http://schemas.android.com/apk/res/android}name') == "android.intent.action.MAIN":
                            return True
    return False
...
if __name__ == '__main__':
    ...
    parser.add_argument("--get-icon-path", action='store_true', help="Output icon path as \"icon\" variable to GitHub Actions.")
